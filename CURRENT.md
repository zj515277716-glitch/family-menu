...
下一步：**T-P04 已验收，待用户裁决提交**；队列与待请示——①**内容轨道 3 决策点**（调查报告 [tools/content-pipeline/docs/recipe-fetch-feasibility-2026-09-06.md](./tools/content-pipeline/docs/recipe-fetch-feasibility-2026-09-06.md)：下厨房有条件可行但详情页 302 JS 挑战、需授权浏览器自动化或人工另存 HTML；小红书自动化不可行、仅人工录入；契约字段扩展 Dish.imageUrl/sourceUrl/sourceSite + origin 枚举、动 shared 须用户批准）②T-P05 必消模糊/别名匹配（先技术方案：别名表 vs 包含匹配 vs 输入联想）③T-P06 做法详情页图文改版（2-3 套方向稿；含 R-3 防抖与上一道/下一道导航）④R-2 挂账：loadMenuViews/hydrateLockedMenu 补 orderBy:{sort:'asc'}（独立小卡）⑤V5 根因挂账：GET feedback 404→200+null。fm-reviewer 建议级遗留 B-2/B-3/B-4（T-P03）后续排期。
