...
下一步：**T-P06 实施提交与否待用户裁决（5 文件 + 卡 + evidence；R-1 loading 卡死小缺陷与 API 投影小卡同批处理待批准）**；T-C05 用料审改（用户人工，证据报告 §8）；代码轨道队列——①**API imageUrl/sourceSite 投影小卡**（含 R-4 mappers orderBy 评估 + lock 响应水合路径设计输入）②R-1 修复（dish 页 useDidShow catch 卡 loading 态）③T-P05 必消模糊/别名匹配（先技术方案）④R-2 挂账：loadMenuViews/hydrateLockedMenu 补 orderBy（可并入①）⑤V5 根因挂账：GET feedback 404→200+null；内容轨道挂账——蚝油菜谱补采、R-9 DRAFT 只读口、R-10 绝对 URL vs 契约张力（知悉项）。fm-reviewer 建议级遗留 B-2/B-3/B-4（T-P03）后续排期。**环境口径更正：cookie 实名 access_token（小写下划线），ACCESS_TOKEN 是环境变量名**。当前 HEAD `46d4787`。
