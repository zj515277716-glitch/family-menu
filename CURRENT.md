...
- 进行中（内容轨道 + 代码轨道并行）：①**T-C05 用料初稿补齐**（2026-09-10，[evidence/T-C05-dev-2026-09-10.md](./evidence/T-C05-dev-2026-09-10.md)）——29 道 FETCHED 菜按常见做法补齐 258 条用料（11 道 uncertain 标记），幂等脚本 apply-ingredients.mjs 入库，19 道既有菜零变化；停「待用户审核」，**29 道用料初稿待用户人工审改**（证据报告 §8 清单）。②**T-P06 详情页 vA 实施**（2026-09-10，[evidence/T-P06-impl-dev-2026-09-10.md](./evidence/T-P06-impl-dev-2026-09-10.md)）——dish 页 vA 落地（5 文件 704+/101-：340px 大图 hero+图上叠信息+换道导航+分组用料+待补齐态+三态防抖 R-3；e-final 屏⑪+design-spec 同步）；typecheck 0/build EXIT=0/真机 13 截图；停「待用户审核」，待 fm-reviewer/fm-verify 复审验收。**卡外发现**：API 菜单快照不投影 imageUrl/sourceSite（真机大图走降级，有图渲染经 Fetch 注入法实证）——需 API 小卡补投影；**流程教训**：两 fm-dev 并行共用同一 PG 互相干扰（T-P06 测试 4 Plan+8 Event 混入 T-C05 对账，主控已删净恢复 54/93），后续并行派发须错开 PG 使用或串行。

下一步：**T-P07 提交与否待用户裁决（4 文件 +38/-5 + 新 spec + 卡 + evidence）+ R-10 升级立卡待批准**；挂账队列——①**R-10（中风险，公网部署 API 前必须处理）**：管线 fetch2dish 改存相对路径 + 存量数据迁移重写 + 前端 <Image> onError 降级兜底；②T-P05 必消模糊/别名匹配（先技术方案）；③建议-1（spec 无 PG 静默 skip 假绿风险 + recommend 投影断言固化进 vitest）、建议-2（types/index.ts L50 旧注释漂移）随后续卡顺带；④CookLog 与 Plan/Event 时区口径不一致（timestamptz 评估，schema 级）；⑤V5 根因挂账：GET feedback 404→200+null；内容轨道——T-C05 用料审改（用户人工 §8）、蚝油菜补采、R-9 DRAFT 只读口。fm-reviewer 建议级遗留 B-2/B-3/B-4（T-P03）后续排期。**环境口径更正：cookie 实名 access_token（小写下划线），ACCESS_TOKEN 是环境变量名**。当前 HEAD `123195f`。
