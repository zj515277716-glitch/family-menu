...
> 快照时间：2026-09-11 ｜ 基线 git HEAD：`75f9d76`（分支 feat/tp-06-menu-expansion；EMPTYFIX 构建产物 39eeb0c6 已部署公网；本文件随 T-P09 提交更新，之后以 git log 为准）
...
- 进行中（内容轨道）：**T-C05 用料初稿补齐**（2026-09-10，[evidence/T-C05-dev-2026-09-10.md](./evidence/T-C05-dev-2026-09-10.md)）——29 道 FETCHED 菜按常见做法补齐 258 条用料（11 道 uncertain 标记），幂等脚本 apply-ingredients.mjs 入库，19 道既有菜零变化；停「待用户审核」，**29 道用料初稿待用户人工审改**（证据报告 §8 清单），改完可重跑 apply-ingredients.mjs 幂等回写。**流程教训（存档）**：两 fm-dev 并行共用同一 PG 互相干扰（T-P06 测试数据曾混入 T-C05 对账，主控删净恢复），后续并行派发须错开 PG 使用或串行。

下一步：**T-P09 已完结（2026-09-11 用户批准提交），下一个代码任务待用户裁决立项**。挂账队列——①**公网部署清单**（R-10 收尾：迁移脚本参数化跑公网 RDS + .env.production 的 TARO_APP_API_BASE_URL 确认 + /images/* 鉴权策略，T-P08 复审建议-2，四项齐备前不建议公网发布新版）；②T-P05 必消模糊/别名匹配（先技术方案）；③建议-1（projection-order.spec 无 PG 静默 skip 假绿 + afterAll cleanup 无 PG 容错（T-P09 fm-dev 遗留）+ recommend 投影断言固化进 vitest）、建议-2（types/index.ts L50 旧注释漂移）随后续卡顺带；④CookLog 与 Plan/Event 时区口径不一致（timestamptz 评估，schema 级）；⑤V5 根因挂账：GET feedback 404→200+null；⑥AC12 性能测试并行模式 flaky（固化串行跑法或调阈值）；⑦NutUI postcss-calc 构建警告（库既有，另立任务）；⑧**imageUrl regex 口径收紧评估**（T-P09 复审 P1：现 `/^\/images\//` 放行 `/images/../secret` 路径穿越形式与无扩展名路径，建议收紧为 `/^\/images\/dishes\/[A-Za-z0-9]+\/\d+\.(webp|jpe?g|png)$/` 或至少排除 `..`——改契约须再走批准）；⑨**契约版本账本分裂收敛**（v0.2~v0.6 记 api.ts L185、v0.7 记 dish.ts 头部 L3-L7，后续收敛回 api.ts 单一入口）；内容轨道——T-C05 用料审改（用户人工 §8）、蚝油菜补采、R-9 DRAFT 只读口。fm-reviewer 建议级遗留 B-2/B-3/B-4（T-P03）后续排期。**环境口径：cookie 实名 access_token（小写下划线），ACCESS_TOKEN 是环境变量名；并行派发须错开 PG 或串行；shared 改 src 后先 build 重建 dist 再跑回归**。
