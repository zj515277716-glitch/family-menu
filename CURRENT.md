...
> 快照时间：2026-09-11 ｜ 基线 git HEAD：`da259a6`（分支 feat/tp-06-menu-expansion；EMPTYFIX 构建产物 39eeb0c6 已部署公网）
...
下一步：**T-P09 契约 imageUrl 口径放宽（进行中，已派发 fm-dev）**——shared DishSchema.imageUrl 放宽为「http(s) 绝对 URL 或 /images/ 站内相对路径」+ import.spec 补相对路径正反用例（消除 T-P08 阻塞-1 测试盲区）+ fetch2dish `--base-url=` 等号形式拦截（T-P08 复审建议-1）；**shared 契约变更已获用户批准（2026-09-11）**。挂账队列——①**公网部署清单**（R-10 收尾：迁移脚本参数化跑公网 RDS + .env.production 的 TARO_APP_API_BASE_URL 确认 + /images/* 鉴权策略，T-P08 复审建议-2，四项齐备前不建议公网发布新版）；②T-P05 必消模糊/别名匹配（先技术方案）；③建议-1（projection-order.spec 无 PG 静默 skip 假绿 + recommend 投影断言固化进 vitest）、建议-2（types/index.ts L50 旧注释漂移）随后续卡顺带；④CookLog 与 Plan/Event 时区口径不一致（timestamptz 评估，schema 级）；⑤V5 根因挂账：GET feedback 404→200+null；⑥AC12 性能测试并行模式 flaky（固化串行跑法或调阈值）；⑦NutUI postcss-calc 构建警告（库既有，另立任务）；内容轨道——T-C05 用料审改（用户人工 §8）、蚝油菜补采、R-9 DRAFT 只读口。fm-reviewer 建议级遗留 B-2/B-3/B-4（T-P03）后续排期。**环境口径：cookie 实名 access_token（小写下划线），ACCESS_TOKEN 是环境变量名；并行派发须错开 PG 或串行**。当前 HEAD `da259a6`（本文件随 T-P08 提交更新，之后以 git log 为准）。
