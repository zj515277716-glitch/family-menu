...
- 进行中（内容轨道 + 设计轨道并行）：①**T-C04 批量采集填充菜谱池**（2026-09-10，[evidence/T-C04-dev-2026-09-10.md](./evidence/T-C04-dev-2026-09-10.md)）——fm-dev 用 batch-fetch.mjs（新增 ~720 行，断点续采/赞数排序/视频预过滤，零新增依赖）30 关键词实采 **29 成功/1 失败**（蚝油生菜候选不足），Dish 19→**48**（+29 全部 FETCHED/DRAFT），87 张图落 apps/api/static/images/dishes/；主控对账复核 EXIT=0；执行中修复搜索页 state 结构适配等 3 问题（均在编排脚本内）。停「待用户审核」。②**T-P06 详情页三方向稿**（2026-09-10，[evidence/T-P06-dev-2026-09-10.md](./evidence/T-P06-dev-2026-09-10.md)）——dish-detail-vA（沉浸大图流）/vB（结构化 checklist）/vC（备菜动线时间条）三 HTML 静稿 + 双态覆盖 + R-3 防抖演示，零业务代码改动，停「待用户审核」待用户选方向后另开实施卡。

下一步：**用户两项裁决**——①T-C04 提交（batch-fetch.mjs + 两卡 + evidence；数据/图片为运行时产物不入库）②T-P06 三方向稿选定（A 沉浸大图流 / B 结构化 checklist / C 备菜动线时间条，可组合，选中后另开实施卡走确认书先行）；代码轨道队列——①T-P05 必消模糊/别名匹配（先技术方案：别名表 vs 包含匹配 vs 输入联想）②T-P06 实施（方向定后）③R-2 挂账：loadMenuViews/hydrateLockedMenu 补 orderBy:{sort:'asc'}（独立小卡）④V5 根因挂账：GET feedback 404→200+null；内容轨道挂账——蚝油菜谱补采、R-9 DRAFT 只读口、R-10 绝对 URL vs 契约张力（知悉项）、xhs-fetch.mjs 候选提取 state 结构适配报备。fm-reviewer 建议级遗留 B-2/B-3/B-4（T-P03）后续排期。当前 HEAD `050d479`。
