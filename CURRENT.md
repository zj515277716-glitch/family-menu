...
- 进行中（内容轨道，与代码轨道并行，WIP=1 不适用）：**T-C01 Dish 契约扩展 + T-C02 小红书 CDP 试采**（2026-09-10 闭环，证据见 [evidence/T-C01-T-C02-2026-09-10.md](./evidence/T-C01-T-C02-2026-09-10.md)）——用户三项裁决（3a 下厨房选 A 浏览器自动化/3b 契约扩展批准/3c 小红书 CDP 采集+人工登录配合）后双 fm-dev 并行交付：T-C01 shared dish 三 optional 字段（imageUrl/sourceUrl/sourceSite）+origin 枚举 FETCHED + prisma 迁移 20260910000000 + import 透传（单测 61/61+57/57、真实 PG 端到端、Dish 基线 19 精确还原）；T-C02 xhs-fetch.mjs 最小 CDP 客户端复用 9222 已登录 Edge（零新增依赖），真实采集 1 篇家常菜笔记五要素 + 3 图（FETCHED_NOT_IMPORTED 未入库）。fm-reviewer 双**通过**（R-4 FETCHED 落库口待入库卡处置/R-5 checksum drift 取证=TP-04 手工登记所致待 resolve/R-6 基线口径 19/C-1 out/ 未入 .gitignore 待裁决/C-2 webp 误名）→ fm-verify V1~V8 全 PASS（22 证据文件 .workflow-verify/tp-c01-verify/）。两卡停「待用户审核」。

下一步：**T-C01/T-C02 待用户裁决（提交/C-1 gitignore/R-5 migrate resolve/T-C03 入库卡立项）**；代码轨道队列——①T-P05 必消模糊/别名匹配（先技术方案：别名表 vs 包含匹配 vs 输入联想）②T-P06 做法详情页图文改版（2-3 套方向稿；含 R-3 防抖与上一道/下一道导航）③R-2 挂账：loadMenuViews/hydrateLockedMenu 补 orderBy:{sort:'asc'}（独立小卡）④V5 根因挂账：GET feedback 404→200+null；内容轨道队列——下厨房抓取卡（用户已授权浏览器自动化，复用 CDP 9222 会话）、T-C03 fetch 内容入库卡（R-4：FETCHED 专用入库路径不削弱双保险；C-2 扩展名修正；图片落服务端静态目录+Dish 存相对 URL）。fm-reviewer 建议级遗留 B-2/B-3/B-4（T-P03）后续排期。
